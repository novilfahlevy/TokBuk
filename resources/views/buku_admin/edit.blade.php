    ...
    <section class="section">
      ...
      <div class="section-body">
        <form action="{{ route('buku.update', ['id' => $buku->id]) }}" method="POST" enctype="multipart/form-data"
          ...
          <div class="row">
            <div class="col-sm-4">
              <img src="{{ asset('images/buku/'.$buku->sampul) }}" id="showgambar"
                style="width:100%; height:400px; background-size: cover" class="img-thumbnail mb-3" />
                  ...
                  <div class="row">
                    ...
                    <div class="col-12 col-md-6">
                      <div class="form-group">
                        ...
                        <select name="id_kategori" id="id_kategpri" class="form-control"
                          value="{{ old('id_kategori') }}" data-live-search="true">
                          <option value='' selected disabled>- Pilih -</option>
                          @foreach($kategori as $jen)
                          <option value="{{ $jen['id'] }}" {{$jen->id == $buku->id_kategori ?  'selected' : ''}}>
                            {{$jen->nama}} </option>
                          @endforeach
                        </select>
                      </div>
                    ...
                    <div class="col-12 col-md-6">
                      <div class="form-group">
                        ...
                        <select name="id_penulis" id="id_penulis" class="form-control" value="{{ old('id_penulis') }}"
                          data-live-search="true">
                          <option value='' disabled selected>- Pilih -</option>
                          @foreach($penulis as $pel)
                          <option value="{{ $pel['id'] }}" {{$pel->id == $buku->id_penulis ?  'selected' : ''}}>
                            {{$pel->nama}} </option>
                          @endforeach
                        </select>
                      </div>
                    ...
                    <div class="col-12 col-md-6">
                      <div class="form-group">
                        ...
                        <select name="id_penerbit" class="form-control" value="{{ old('id_penerbit') }}"
                          data-live-search="true">
                          <option value='' selected disabled>- Pilih -</option>
                          @foreach($penerbit as $per)
                          <option value="{{ $per['id'] }}" {{$per->id == $buku->id_penerbit ?  'selected' : ''}}>
                            {{$per->nama}} </option>
                          @endforeach
                        </select>
                      </div>
                    ...
                    <div class="col-12 col-md-6">
                      <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="tahun_terbit" name="tahun_terbit"
                          value="{{ $buku->tahun_terbit }}" min="0">
                      </div>
                    </div>
                    <div class="col-12 col-md-6">
                      <div class="form-group">
                        <label for="id_lokasi">Lokasi</label>
                        <select name="id_lokasi" id="id_lokasi" class="form-control" value="{{ old('id_lokasi') }}"
                          data-live-search="true">
                          <option value='' selected disabled>- Pilih -</option>
                          @foreach($lokasi as $lok)
                          <option value="{{ $lok['id'] }}" {{$lok->id == $buku->id_lokasi ?  'selected' : ''}}>
                            {{$lok->nama}} </option>
                          @endforeach
                        </select>
                      </div>
